...
def bot(query: str, options: str, test_case: Dict[str, str], max_new_tokens: int=32,  context_score: int=0.35) -> str: 
    ...
    context = []
    for key in keywords:
        inf = db_context.similarity_search_with_relevance_scores(key, k=5, score_threshold=context_score)
        if len(inf) != 0:
    ...
    while is_invalid_format(choices) or (not check_element(options_list, choices)):
        ...
        print(choices)
    print(is_invalid_format(choices), (not check_element(options_list, choices)))
